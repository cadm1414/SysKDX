...
public class dlg_busq_subfamilia extends javax.swing.JDialog {

    pnl_grid_busq_subfamilia lo_pnl_grid_busq_subfamilia = new pnl_grid_busq_subfamilia();
    DefaultTableModel lm_modelo;
    ResultSet lq_rs;
    public String ls_codigo_subfamilia;

    public dlg_busq_subfamilia(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        formulario();
        datos_tabla();
    ...
    private void formulario() {
        lo_pnl_grid_busq_subfamilia.setBounds(0, 0, 300, 220);
        PNL_grid.add(lo_pnl_grid_busq_subfamilia);

        TXT_dato.setDocument(new fnc_txt_mayuscula());

        lo_pnl_grid_busq_subfamilia.TBL_subfamilia.addMouseListener(MouseEvnt);
        lo_pnl_grid_busq_subfamilia.TBL_subfamilia.addKeyListener(KeyEvnt);
        TXT_dato.addKeyListener(KeyEvnt);
    }

    private void datos_tabla() {
        int a = 0;
        lm_modelo = (DefaultTableModel) lo_pnl_grid_busq_subfamilia.TBL_subfamilia.getModel();
        try {
            lq_rs = go_dao_subfamilia.SLT_grid_subfamilia();
            if (lq_rs != null) {
                do {
                    lm_modelo.addRow(new Object[]{""});
                    for (int x = 0; x < 2; x++) {
                        lo_pnl_grid_busq_subfamilia.TBL_subfamilia.setValueAt(lq_rs.getString(x + 1), a, x);
                    }
                    a++;
                } while (lq_rs.next());
            }
    ...
    public void retorna() {
        ls_codigo_subfamilia = lo_pnl_grid_busq_subfamilia.TBL_subfamilia.getValueAt(lo_pnl_grid_busq_subfamilia.TBL_subfamilia.getSelectedRow(), 0).toString();
        this.dispose();
    }
    ...
    KeyListener KeyEvnt = new KeyListener() {
        ...
        public void keyPressed(KeyEvent ke) {
            if (ke.getKeyCode() == KeyEvent.VK_ENTER) {
                if (ke.getSource() == TXT_dato) {
                    if (lo_pnl_grid_busq_subfamilia.TBL_subfamilia.getRowCount() != 0) {
                        lo_pnl_grid_busq_subfamilia.TBL_subfamilia.requestFocus();
                        lo_pnl_grid_busq_subfamilia.TBL_subfamilia.changeSelection(0, 0, false, false);
                    } else {
                        TXT_dato.requestFocus();
                    }
                }
                if (ke.getSource() == lo_pnl_grid_busq_subfamilia.TBL_subfamilia) {
                    retorna();
                }
            }
        ...
        public void keyReleased(KeyEvent ke) {
            if (ke.getSource() == TXT_dato) {
                go_fnc_filtrar_tablas.filtro(lm_modelo, lo_pnl_grid_busq_subfamilia.TBL_subfamilia, TXT_dato.getText(), 1);
            }
        }
    ...
    MouseListener MouseEvnt = new MouseListener() {
        ...
        public void mouseClicked(MouseEvent me) {
            if (me.getSource() == lo_pnl_grid_busq_subfamilia.TBL_subfamilia && me.getClickCount() == 2) {
                retorna();
            }
        }
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                dlg_busq_subfamilia dialog = new dlg_busq_subfamilia(new javax.swing.JFrame(), true);
