...
public class dlg_almacen_x_permiso extends javax.swing.JDialog {

    pnl_grid_almacen_x_permiso lo_pnl_grid_almacen_x_permiso = new pnl_grid_almacen_x_permiso();
    DefaultTableModel lm_modelo;
    ResultSet lq_rs;
    public String ls_codigo_almacen;

    public dlg_almacen_x_permiso(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        formulario();
        datos_tabla();
    ...
    private void formulario() {
        lo_pnl_grid_almacen_x_permiso.setBounds(0, 0, 300, 220);
        PNL_grid.add(lo_pnl_grid_almacen_x_permiso);

        TXT_dato.setDocument(new fnc_txt_mayuscula());

        lo_pnl_grid_almacen_x_permiso.TBL_almacen_acceso.addMouseListener(MouseEvnt);
        lo_pnl_grid_almacen_x_permiso.TBL_almacen_acceso.addKeyListener(KeyEvnt);
        ...
        try {
            ...
            if (lq_rs != null) {
                do {
                    ...
                    for (int x = 0; x < 2; x++) {
                        lo_pnl_grid_almacen_x_permiso.TBL_almacen_acceso.setValueAt(lq_rs.getString(x + 1), a, x);
                    }
                    a++;
                } while (lq_rs.next());
            }
    ...
    public void retorna() {
        ls_codigo_almacen = lo_pnl_grid_almacen_x_permiso.TBL_almacen_acceso.getValueAt(lo_pnl_grid_almacen_x_permiso.TBL_almacen_acceso.getSelectedRow(), 0).toString();
        this.dispose();
    }
    ...
    KeyListener KeyEvnt = new KeyListener() {
        ...
        public void keyReleased(KeyEvent ke) {
            if (ke.getSource() == TXT_dato) {
                go_fnc_filtrar_tablas.filtro(lm_modelo, lo_pnl_grid_almacen_x_permiso.TBL_almacen_acceso, TXT_dato.getText(), 1);
            }
        }
    ...
    MouseListener MouseEvnt = new MouseListener() {
        ...
        public void mouseClicked(MouseEvent me) {
            if (me.getSource() == lo_pnl_grid_almacen_x_permiso.TBL_almacen_acceso && me.getClickCount() == 2) {
                retorna();
            }
        }
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                dlg_almacen_x_permiso dialog = new dlg_almacen_x_permiso(new javax.swing.JFrame(), true);
