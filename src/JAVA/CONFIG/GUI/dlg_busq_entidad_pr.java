...
public class dlg_busq_entidad_pr extends javax.swing.JDialog {
    ...
    public dlg_busq_entidad_pr(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        formulario();
        datos_tabla();
    ...
    private void datos_tabla() {
        ...
        go_fnc_operaciones_campos.ancho_columna(lo_pnl_grid_busq_entidad.TBL_entidad, 3, 0);
        try {
            lq_rs = go_dao_entidad.SLT_grid_entidad_pr(gs_parametros[0], gs_parametros[1]);
            limpia_parametros();
            if (lq_rs != null) {
                do {
                    lm_modelo.addRow(new Object[]{""});
                    lo_pnl_grid_busq_entidad.TBL_entidad.setValueAt(lq_rs.getString(1), a, 0);
                    lo_pnl_grid_busq_entidad.TBL_entidad.setValueAt(lq_rs.getString(2), a, 1);
                    lo_pnl_grid_busq_entidad.TBL_entidad.setValueAt(lq_rs.getString(5), a, 2);                    
                    a++;
                } while (lq_rs.next());
            }
        } catch (Exception e) {
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                dlg_busq_entidad_pr dialog = new dlg_busq_entidad_pr(new javax.swing.JFrame(), true);
