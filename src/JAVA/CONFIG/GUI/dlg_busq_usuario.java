...
public class dlg_busq_usuario extends javax.swing.JDialog {

    pnl_grid_busq_usuario lo_pnl_grid_busq_usuario = new pnl_grid_busq_usuario();
    DefaultTableModel lm_modelo;
    ResultSet lq_rs;
    public String ls_codigo_usuario;

    public dlg_busq_usuario(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        formulario();
        datos_tabla();
    ...
    private void formulario() {
        lo_pnl_grid_busq_usuario.setBounds(0, 0, 300, 220);
        PNL_grid.add(lo_pnl_grid_busq_usuario);

        TXT_dato.setDocument(new fnc_txt_mayuscula());

        lo_pnl_grid_busq_usuario.TBL_usuario.addMouseListener(MouseEvnt);
        lo_pnl_grid_busq_usuario.TBL_usuario.addKeyListener(KeyEvnt);
        TXT_dato.addKeyListener(KeyEvnt);
    }

    private void datos_tabla() {
        int a = 0;
        lm_modelo = (DefaultTableModel) lo_pnl_grid_busq_usuario.TBL_usuario.getModel();
        try {
            lq_rs = go_dao_usuario.SLT_grid_usuario();
            if (lq_rs != null) {
                do {
                    lm_modelo.addRow(new Object[]{""});
                    for (int x = 0; x < 3; x++) {
                        lo_pnl_grid_busq_usuario.TBL_usuario.setValueAt(lq_rs.getString(x + 1), a, x);
                    }
                    a++;
                } while (lq_rs.next());
            }
    ...
    public void retorna() {
        ls_codigo_usuario = lo_pnl_grid_busq_usuario.TBL_usuario.getValueAt(lo_pnl_grid_busq_usuario.TBL_usuario.getSelectedRow(), 0).toString();
        this.dispose();
    }
    ...
    KeyListener KeyEvnt = new KeyListener() {
        ...
        public void keyPressed(KeyEvent ke) {
            if (ke.getKeyCode() == KeyEvent.VK_ENTER) {
                if (ke.getSource() == TXT_dato) {
                    if (lo_pnl_grid_busq_usuario.TBL_usuario.getRowCount() != 0) {
                        lo_pnl_grid_busq_usuario.TBL_usuario.requestFocus();
                        lo_pnl_grid_busq_usuario.TBL_usuario.changeSelection(0, 0, false, false);
                    } else {
                        TXT_dato.requestFocus();
                    }
                }
                if (ke.getSource() == lo_pnl_grid_busq_usuario.TBL_usuario) {
                    retorna();
                }
            }
        ...
        public void keyReleased(KeyEvent ke) {
            if (ke.getSource() == TXT_dato) {
                go_fnc_filtrar_tablas.filtro(lm_modelo, lo_pnl_grid_busq_usuario.TBL_usuario, TXT_dato.getText(), 1);
            }
        }
    ...
    MouseListener MouseEvnt = new MouseListener() {
        ...
        public void mouseClicked(MouseEvent me) {
            if (me.getSource() == lo_pnl_grid_busq_usuario.TBL_usuario && me.getClickCount() == 2) {
                retorna();
            }
        }
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                dlg_busq_usuario dialog = new dlg_busq_usuario(new javax.swing.JFrame(), true);
