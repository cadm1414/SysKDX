...
public class dlg_busq_liquidacion extends javax.swing.JDialog {

    pnl_grid_busq_liquidacion lo_pnl_grid_busq_liquidacion = new pnl_grid_busq_liquidacion();
    DefaultTableModel lm_modelo;
    ResultSet lq_rs;
    public String ls_codigo;
    String ls_codigo_sucursal, ls_fecha_ini, ls_fecha_fin, ls_serie, ls_es_facturado, ls_codigo_doc_ref, ls_estado;
    String ls_modulo = "DISTBR", ls_capa = "GUI", ls_clase = "dlg_busq_liquidacion";

    public dlg_busq_liquidacion(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        formulario();
        get_parametros();
    ...
    private void formulario() {
        lo_pnl_grid_busq_liquidacion.setBounds(0, 0, 300, 220);
        PNL_grid.add(lo_pnl_grid_busq_liquidacion);

        TXT_dato.setDocument(new fnc_txt_mayuscula());

        lo_pnl_grid_busq_liquidacion.TBL_liquidacion.addMouseListener(MouseEvnt);
        lo_pnl_grid_busq_liquidacion.TBL_liquidacion.addKeyListener(KeyEvnt);
    ...
    private void datos_tabla() {
        int a = 0;
        lm_modelo = (DefaultTableModel) lo_pnl_grid_busq_liquidacion.TBL_liquidacion.getModel();
        try {
            lq_rs = go_dao_liquidacion.SLT_grid_liquidacion(ls_codigo_sucursal, ls_fecha_ini, ls_fecha_fin, ls_estado);
            if (lq_rs != null) {
                do {
                    lm_modelo.addRow(new Object[]{""});
                    for (int x = 0; x < 4; x++) {
                        if (x == 1) {
                            lo_pnl_grid_busq_liquidacion.TBL_liquidacion.setValueAt(go_fnc_operaciones_campos.recupera_fecha_formato(lq_rs.getString(x + 1)), a, x);
                        } else {
                            lo_pnl_grid_busq_liquidacion.TBL_liquidacion.setValueAt(lq_rs.getString(x + 1), a, x);
                        }
    ...
    public void limpia_tabla() {
        DefaultTableModel modelo = (DefaultTableModel) lo_pnl_grid_busq_liquidacion.TBL_liquidacion.getModel();
    ...
    public void retorna() {
        ls_codigo = lo_pnl_grid_busq_liquidacion.TBL_liquidacion.getValueAt(lo_pnl_grid_busq_liquidacion.TBL_liquidacion.getSelectedRow(), 0).toString();
        this.dispose();
    }
    ...
    KeyListener KeyEvnt = new KeyListener() {
        ...
        public void keyPressed(KeyEvent ke) {
            if (ke.getKeyCode() == KeyEvent.VK_ENTER) {
                ...
                if (ke.getSource() == TXT_dato) {
                    if (lo_pnl_grid_busq_liquidacion.TBL_liquidacion.getRowCount() != 0) {
                        lo_pnl_grid_busq_liquidacion.TBL_liquidacion.requestFocus();
                        lo_pnl_grid_busq_liquidacion.TBL_liquidacion.changeSelection(0, 0, false, false);
                    } else {
                        TXT_dato.requestFocus();
                    }
                }
                if (ke.getSource() == lo_pnl_grid_busq_liquidacion.TBL_liquidacion) {
                    retorna();
                }
            }
        ...
        public void keyReleased(KeyEvent ke) {
            if (ke.getSource() == TXT_dato) {
                go_fnc_filtrar_tablas.filtro(lm_modelo, lo_pnl_grid_busq_liquidacion.TBL_liquidacion, TXT_dato.getText(), 1);
            }
        }
    ...
    MouseListener MouseEvnt = new MouseListener() {
        ...
        public void mouseClicked(MouseEvent me) {
            if (me.getSource() == lo_pnl_grid_busq_liquidacion.TBL_liquidacion && me.getClickCount() == 2) {
                retorna();
            }
        }
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                dlg_busq_liquidacion dialog = new dlg_busq_liquidacion(new javax.swing.JFrame(), true);
