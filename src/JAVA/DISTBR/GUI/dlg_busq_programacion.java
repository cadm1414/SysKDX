...
public class dlg_busq_programacion extends javax.swing.JDialog {

    pnl_grid_busq_programacion lo_pnl_grid_busq_programacion = new pnl_grid_busq_programacion();
    DefaultTableModel lm_modelo;
    ResultSet lq_rs;
    public String ls_codigo;
    String ls_codigo_sucursal, ls_fecha_ini, ls_fecha_fin, ls_serie, ls_es_facturado, ls_codigo_doc_ref, ls_es_liquidado;
    String ls_modulo = "DISTBR", ls_capa = "GUI", ls_clase = "dlg_busq_programacion";

    public dlg_busq_programacion(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        formulario();
        get_parametros();
    ...
    private void get_parametros() {
        ...
        ls_es_liquidado = gs_parametros[3];
    ...
    private void formulario() {
        lo_pnl_grid_busq_programacion.setBounds(0, 0, 300, 220);
        PNL_grid.add(lo_pnl_grid_busq_programacion);

        TXT_dato.setDocument(new fnc_txt_mayuscula());

        lo_pnl_grid_busq_programacion.TBL_programacion.addMouseListener(MouseEvnt);
        lo_pnl_grid_busq_programacion.TBL_programacion.addKeyListener(KeyEvnt);
    ...
    private void datos_tabla() {
        int a = 0;
        lm_modelo = (DefaultTableModel) lo_pnl_grid_busq_programacion.TBL_programacion.getModel();
        try {
            lq_rs = go_dao_programacion.SLT_grid_programacion(ls_codigo_sucursal, ls_fecha_ini, ls_fecha_fin, ls_es_liquidado);
            if (lq_rs != null) {
                do {
                    lm_modelo.addRow(new Object[]{""});
                    for (int x = 0; x < 4; x++) {
                        if (x == 1) {
                            lo_pnl_grid_busq_programacion.TBL_programacion.setValueAt(go_fnc_operaciones_campos.recupera_fecha_formato(lq_rs.getString(x + 1)), a, x);
                        } else {
                            lo_pnl_grid_busq_programacion.TBL_programacion.setValueAt(lq_rs.getString(x + 1), a, x);
                        }
    ...
    public void limpia_tabla() {
        DefaultTableModel modelo = (DefaultTableModel) lo_pnl_grid_busq_programacion.TBL_programacion.getModel();
        ...
        public void keyPressed(KeyEvent ke) {
            if (ke.getKeyCode() == KeyEvent.VK_ENTER) {
                ...
                if (ke.getSource() == TXT_dato) {
                    if (lo_pnl_grid_busq_programacion.TBL_programacion.getRowCount() != 0) {
                        lo_pnl_grid_busq_programacion.TBL_programacion.requestFocus();
                        lo_pnl_grid_busq_programacion.TBL_programacion.changeSelection(0, 0, false, false);
                    } else {
                        TXT_dato.requestFocus();
                    }
                }
                if (ke.getSource() == lo_pnl_grid_busq_programacion.TBL_programacion) {
                    retorna();
                }
            }
    ...
    MouseListener MouseEvnt = new MouseListener() {
        ...
        public void mouseClicked(MouseEvent me) {
            if (me.getSource() == lo_pnl_grid_busq_programacion.TBL_programacion && me.getClickCount() == 2) {
                retorna();
            }
        }
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                dlg_busq_programacion dialog = new dlg_busq_programacion(new javax.swing.JFrame(), true);
