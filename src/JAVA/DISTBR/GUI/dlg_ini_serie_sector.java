...
public class dlg_ini_serie_sector extends javax.swing.JDialog {
    
    pnl_ini_serie_sector lo_pnl_ini_serie_sector = new pnl_ini_serie_sector();
    ...
    public dlg_ini_serie_sector(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        formulario();
    ...
    private void formulario() {
        lo_pnl_ini_serie_sector.setBounds(10, 10, 500, 120);
        lo_pnl_aceptar_cancelar.setBounds(100, 120, 200, 50);
        
        ls_tipo_documento = gs_parametros[0];
        
        this.add(lo_pnl_ini_serie_sector);
        this.add(lo_pnl_aceptar_cancelar);
        
        lo_pnl_ini_serie_sector.TXT_codigo.addKeyListener(KeyEvnt);
        lo_pnl_ini_serie_sector.CBX_serie.addKeyListener(KeyEvnt);
        lo_pnl_ini_serie_sector.CBX_sector.addKeyListener(KeyEvnt);
    ...
    private void get_sector_distribucion() {
        ...
        if (lq_rs != null) {
            go_cbx_trato_datos.recupera_valor(20, lq_rs, lo_pnl_ini_serie_sector.CBX_sector);
        }
    }
    
    private void limpia_datos() {
        lo_pnl_ini_serie_sector.TXT_codigo.setText("");
        lo_pnl_ini_serie_sector.TXT_nombre.setText("");
        lo_pnl_ini_serie_sector.CBX_serie.removeAllItems();
        lo_pnl_ini_serie_sector.TXT_codigo.requestFocus();
    }
    ...
    private void get_serie(String codigo) {
        ...
        try {
            ...
            if (lq_rs != null) {
                do {
                    lo_pnl_ini_serie_sector.CBX_serie.addItem(lq_rs.getString(1));
                } while (lq_rs.next());
            } else {
    ...
    private void get_descripcion_sucursal(String codigo) {
        try {
            ...
            if (lq_rs != null) {
                lo_pnl_ini_serie_sector.TXT_codigo.setText(lq_rs.getString(1));
                lo_pnl_ini_serie_sector.TXT_nombre.setText(lq_rs.getString(2));
    ...
    private void muestra_jif() {
        if (go_fnc_operaciones_campos.campo_blanco(lo_pnl_ini_serie_sector.TXT_codigo)) {
    ...
    private void evt_aceptar() {
        if (lo_pnl_ini_serie_sector.CBX_serie.getItemCount() > 0) {
            gs_parametros[2] = lo_pnl_ini_serie_sector.CBX_serie.getSelectedItem().toString();
            get_descripcion_sucursal(lo_pnl_ini_serie_sector.TXT_codigo.getText().trim());
            muestra_jif();
        } else {
    ...
    KeyListener KeyEvnt = new KeyListener() {
        ...
        public void keyPressed(KeyEvent ke) {
            if (ke.getKeyCode() == KeyEvent.VK_F5) {
                if (ke.getSource() == lo_pnl_ini_serie_sector.TXT_codigo) {
                    evt_f5();
                }
            }
            ...
            if (ke.getKeyCode() == KeyEvent.VK_ENTER) {
                if (ke.getSource() == lo_pnl_ini_serie_sector.TXT_codigo && go_fnc_operaciones_campos.cant_caracter(lo_pnl_ini_serie_sector.TXT_codigo.getText().trim(), 1, 4)) {
                    lo_pnl_ini_serie_sector.CBX_serie.requestFocus();
                    get_descripcion_sucursal(lo_pnl_ini_serie_sector.TXT_codigo.getText().trim());
                }
                if (ke.getSource() == lo_pnl_ini_serie_sector.CBX_serie) {
                    lo_pnl_ini_serie_sector.CBX_sector.requestFocus();
                }
                if (ke.getSource() == lo_pnl_ini_serie_sector.CBX_sector) {
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                dlg_ini_serie_sector dialog = new dlg_ini_serie_sector(new javax.swing.JFrame(), true);
