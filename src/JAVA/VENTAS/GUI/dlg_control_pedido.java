...
public class dlg_control_pedido extends javax.swing.JDialog {

    pnl_control_pedido lo_pnl_control_pedido = new pnl_control_pedido();
    ...
    public dlg_control_pedido(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        formulario();
    }
    ...
    private void formulario() {
        lo_pnl_control_pedido.setBounds(10, 10, 450, 120);
        lo_pnl_aceptar_cancelar.setBounds(120, 120, 200, 50);

        this.add(lo_pnl_control_pedido);
        this.add(lo_pnl_aceptar_cancelar);

        lo_pnl_control_pedido.TXT_fecha_ini.setText(gs_dia + gs_mes + gs_periodo);
        lo_pnl_control_pedido.TXT_fecha_fin.setText(gs_dia + gs_mes + gs_periodo);

        lo_pnl_control_pedido.TXT_codigo.addKeyListener(KeyEvnt);
        lo_pnl_control_pedido.TXT_fecha_ini.addKeyListener(KeyEvnt);
        lo_pnl_control_pedido.TXT_fecha_fin.addKeyListener(KeyEvnt);
        lo_pnl_control_pedido.CBX_tipo.addKeyListener(KeyEvnt);
    ...
    private void get_descripcion_sucursal(String codigo) {
        try {
            ...
            if (lq_rs != null) {
                lo_pnl_control_pedido.TXT_codigo.setText(lq_rs.getString(1));
                lo_pnl_control_pedido.TXT_nombre.setText(lq_rs.getString(2));
            } else {
    ...
    private boolean valida_campos() {
        boolean resp = false;
        if (go_fnc_operaciones_campos.campo_blanco(lo_pnl_control_pedido.TXT_codigo) && go_fnc_operaciones_campos.campo_blanco(lo_pnl_control_pedido.TXT_nombre)) {
            if (valida_fecha(lo_pnl_control_pedido.TXT_fecha_ini)) {
                if (valida_fecha(lo_pnl_control_pedido.TXT_fecha_fin)) {
                    if (go_fnc_operaciones_campos.compara_fechas(lo_pnl_control_pedido.TXT_fecha_ini.getText(), lo_pnl_control_pedido.TXT_fecha_fin.getText()) <= 0) {
                        resp = true;
                    } else {
            ...
            lo_pnl_control_pedido.TXT_codigo.requestFocus();
        }
        return resp;
    }
    ...
    private void muestra_rpt() {
        ...
        parametros.put("codigo_sucursal", lo_pnl_control_pedido.TXT_codigo.getText());
        parametros.put("fecha_ini", go_fnc_operaciones_campos.formarto_date(lo_pnl_control_pedido.TXT_fecha_ini.getText()));
        ...
        switch (lo_pnl_control_pedido.CBX_tipo.getSelectedIndex()) {
            case 0:
                go_muestra_reporte_ventas.reporte_pestania("rpt_control_pedido.jasper", parametros, "CONTROL PEDIDOS", 0);
                break;
            case 1:
    ...
    KeyListener KeyEvnt = new KeyListener() {
        ...
        public void keyPressed(KeyEvent ke) {
            if (ke.getKeyCode() == KeyEvent.VK_F5) {
                if (ke.getSource() == lo_pnl_control_pedido.TXT_codigo) {
                    evt_f5();
                }
            }
            ...
            if (ke.getKeyCode() == KeyEvent.VK_ENTER) {
                if (ke.getSource() == lo_pnl_control_pedido.TXT_codigo && go_fnc_operaciones_campos.campo_blanco(lo_pnl_control_pedido.TXT_codigo)) {
                    lo_pnl_control_pedido.TXT_codigo.setText(go_fnc_operaciones_campos.completa_digitos(lo_pnl_control_pedido.TXT_codigo.getText().trim(), "0", 4));
                    getFocusOwner().transferFocus();
                    get_descripcion_sucursal(lo_pnl_control_pedido.TXT_codigo.getText().trim());
                }
                if (ke.getSource() == lo_pnl_control_pedido.TXT_fecha_ini && !lo_pnl_control_pedido.TXT_fecha_ini.getText().trim().equalsIgnoreCase("/  /")) {
                    if (valida_fecha(lo_pnl_control_pedido.TXT_fecha_ini)) {
                        getFocusOwner().transferFocus();
                    }
                }
                if (ke.getSource() == lo_pnl_control_pedido.TXT_fecha_fin && !lo_pnl_control_pedido.TXT_fecha_fin.getText().trim().equalsIgnoreCase("/  /")) {
                    if (valida_fecha(lo_pnl_control_pedido.TXT_fecha_fin)) {
                        if (go_fnc_operaciones_campos.compara_fechas(lo_pnl_control_pedido.TXT_fecha_ini.getText(), lo_pnl_control_pedido.TXT_fecha_fin.getText()) <= 0) {
                            getFocusOwner().transferFocus();
                        } else {
                ...
                if (ke.getSource() == lo_pnl_control_pedido.CBX_tipo) {
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                dlg_control_pedido dialog = new dlg_control_pedido(new javax.swing.JFrame(), true);
