...
public class dlg_busq_pedido extends javax.swing.JDialog {

    pnl_grid_busq_pedido lo_pnl_grid_pedido = new pnl_grid_busq_pedido();
    DefaultTableModel lm_modelo;
    ResultSet lq_rs;
    public String ls_codigo;
    String ls_codigo_sucursal, ls_fecha_ini, ls_fecha_fin, ls_serie;
    ...
    private void formulario() {
        lo_pnl_grid_pedido.setBounds(0, 0, 300, 220);
        PNL_grid.add(lo_pnl_grid_pedido);

        TXT_dato.setDocument(new fnc_txt_mayuscula());

        lo_pnl_grid_pedido.TBL_pedidos.addMouseListener(MouseEvnt);
        lo_pnl_grid_pedido.TBL_pedidos.addKeyListener(KeyEvnt);
    ...
    private void datos_tabla() {
        ...
        try {
            lq_rs = go_dao_pedido.SLT_grid_pedido(ls_codigo_sucursal, ls_fecha_ini, ls_fecha_fin, ls_serie);
            if (lq_rs != null) {
                do {
                    lm_modelo.addRow(new Object[]{""});
                    for (int x = 0; x < 3; x++) {
                        if (x == 0) {
                            lo_pnl_grid_pedido.TBL_pedidos.setValueAt(go_fnc_operaciones_campos.recupera_fecha_formato(lq_rs.getString(x + 1)), a, x);
                        } else {
                            lo_pnl_grid_pedido.TBL_pedidos.setValueAt(lq_rs.getString(x + 1), a, x);
                        }
    ...
    public void limpia_tabla() {
        DefaultTableModel modelo = (DefaultTableModel) lo_pnl_grid_pedido.TBL_pedidos.getModel();
    ...
    public void retorna() {
        ls_codigo = lo_pnl_grid_pedido.TBL_pedidos.getValueAt(lo_pnl_grid_pedido.TBL_pedidos.getSelectedRow(), 1).toString();
        this.dispose();
    }
    ...
    KeyListener KeyEvnt = new KeyListener() {
        ...
        public void keyPressed(KeyEvent ke) {
            if (ke.getKeyCode() == KeyEvent.VK_ENTER) {
                ...
                if (ke.getSource() == TXT_dato) {
                    if (lo_pnl_grid_pedido.TBL_pedidos.getRowCount() != 0) {
                        lo_pnl_grid_pedido.TBL_pedidos.requestFocus();
                        lo_pnl_grid_pedido.TBL_pedidos.changeSelection(0, 0, false, false);
                    } else {
                        TXT_dato.requestFocus();
                    }
                }
                if (ke.getSource() == lo_pnl_grid_pedido.TBL_pedidos) {
                    retorna();
                }
            }
        ...
        public void keyReleased(KeyEvent ke) {
            if (ke.getSource() == TXT_dato) {
                go_fnc_filtrar_tablas.filtro(lm_modelo, lo_pnl_grid_pedido.TBL_pedidos, TXT_dato.getText(), 1);
            }
        }
    ...
    MouseListener MouseEvnt = new MouseListener() {
        ...
        public void mouseClicked(MouseEvent me) {
            if (me.getSource() == lo_pnl_grid_pedido.TBL_pedidos && me.getClickCount() == 2) {
                retorna();
            }
        }
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                dlg_busq_pedido dialog = new dlg_busq_pedido(new javax.swing.JFrame(), true);
