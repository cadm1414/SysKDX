...
public class dlg_rpt_registro_ventas extends javax.swing.JDialog {

    pnl_rpt_registro_ventas lo_pnl_rpt_registro_ventas = new pnl_rpt_registro_ventas();
    ...
    public dlg_rpt_registro_ventas(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        formulario();
    }
    ...
    private void formulario() {
        lo_pnl_rpt_registro_ventas.setBounds(10, 10, 450, 90);
        lo_pnl_aceptar_cancelar.setBounds(120, 90, 200, 50);
        ...
        lo_pnl_rpt_registro_ventas.TXT_codigo.addKeyListener(KeyEvnt);
        lo_pnl_rpt_registro_ventas.TXT_fecha_ini.addKeyListener(KeyEvnt);
        lo_pnl_rpt_registro_ventas.TXT_fecha_fin.addKeyListener(KeyEvnt);
    ...
    private void get_descripcion_sucursal(String codigo) {
        try {
            ...
            if (lq_rs != null) {
                lo_pnl_rpt_registro_ventas.TXT_codigo.setText(lq_rs.getString(1));
                lo_pnl_rpt_registro_ventas.TXT_nombre.setText(lq_rs.getString(2));
            } else {
    ...
    private boolean valida_campos() {
        boolean resp = false;
        if (go_fnc_operaciones_campos.campo_blanco(lo_pnl_rpt_registro_ventas.TXT_codigo) && go_fnc_operaciones_campos.campo_blanco(lo_pnl_rpt_registro_ventas.TXT_nombre)) {
            if (valida_fecha(lo_pnl_rpt_registro_ventas.TXT_fecha_ini)) {
                if (valida_fecha(lo_pnl_rpt_registro_ventas.TXT_fecha_fin)) {
                    if (go_fnc_operaciones_campos.compara_fechas(lo_pnl_rpt_registro_ventas.TXT_fecha_ini.getText(), lo_pnl_rpt_registro_ventas.TXT_fecha_fin.getText()) <= 0) {
                        resp = true;
                    } else {
            ...
            lo_pnl_rpt_registro_ventas.TXT_codigo.requestFocus();
        }
        return resp;
    }
    ...
    private void muestra_rpt() {
        ...
        parametros.put("codigo_sucursal", lo_pnl_rpt_registro_ventas.TXT_codigo.getText());
        parametros.put("fecha_ini", go_fnc_operaciones_campos.formarto_date(lo_pnl_rpt_registro_ventas.TXT_fecha_ini.getText()));
        parametros.put("fecha_fin", go_fnc_operaciones_campos.formarto_date(lo_pnl_rpt_registro_ventas.TXT_fecha_fin.getText()));
    ...
    KeyListener KeyEvnt = new KeyListener() {
        ...
        public void keyPressed(KeyEvent ke) {
            if (ke.getKeyCode() == KeyEvent.VK_F5) {
                if (ke.getSource() == lo_pnl_rpt_registro_ventas.TXT_codigo) {
                    evt_f5();
                }
            }
            ...
            if (ke.getKeyCode() == KeyEvent.VK_ENTER) {
                if (ke.getSource() == lo_pnl_rpt_registro_ventas.TXT_codigo && go_fnc_operaciones_campos.campo_blanco(lo_pnl_rpt_registro_ventas.TXT_codigo)) {
                    lo_pnl_rpt_registro_ventas.TXT_codigo.setText(go_fnc_operaciones_campos.completa_digitos(lo_pnl_rpt_registro_ventas.TXT_codigo.getText().trim(), "0", 4));
                    getFocusOwner().transferFocus();
                    get_descripcion_sucursal(lo_pnl_rpt_registro_ventas.TXT_codigo.getText().trim());
                }
                if (ke.getSource() == lo_pnl_rpt_registro_ventas.TXT_fecha_ini && !lo_pnl_rpt_registro_ventas.TXT_fecha_ini.getText().trim().equalsIgnoreCase("/  /")) {
                    if (valida_fecha(lo_pnl_rpt_registro_ventas.TXT_fecha_ini)) {
                        getFocusOwner().transferFocus();
                    }
                }
                if (ke.getSource() == lo_pnl_rpt_registro_ventas.TXT_fecha_fin && !lo_pnl_rpt_registro_ventas.TXT_fecha_fin.getText().trim().equalsIgnoreCase("/  /")) {
                    if (valida_fecha(lo_pnl_rpt_registro_ventas.TXT_fecha_fin)) {
                        if (go_fnc_operaciones_campos.compara_fechas(lo_pnl_rpt_registro_ventas.TXT_fecha_ini.getText(), lo_pnl_rpt_registro_ventas.TXT_fecha_fin.getText()) <= 0) {
                            getFocusOwner().transferFocus();
                        } else {
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                dlg_rpt_registro_ventas dialog = new dlg_rpt_registro_ventas(new javax.swing.JFrame(), true);
