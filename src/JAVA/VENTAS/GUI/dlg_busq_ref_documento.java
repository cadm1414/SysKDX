...
public class dlg_busq_ref_documento extends javax.swing.JDialog {
    ...
    private void formulario() {
        lo_pnl_grid_busq_ref_documento.setBounds(0, 0, 550, 220);

        PNL_grid.add(lo_pnl_grid_busq_ref_documento);
        TXT_dato.setDocument(new fnc_txt_mayuscula());

        lo_pnl_grid_busq_ref_documento.TBL_referencia.addMouseListener(MouseEvnt);
        lo_pnl_grid_busq_ref_documento.TBL_referencia.addKeyListener(KeyEvnt);
        ...
        try {
            ...
            if (lq_rs != null) {
                do {
                    ...
                    for (int x = 0; x < 5; x++) {
                        if (x == 1) {
                            lo_pnl_grid_busq_ref_documento.TBL_referencia.setValueAt(go_fnc_operaciones_campos.recupera_fecha_formato(lq_rs.getString(x + 1)), a, x);
                        } else {
    ...
    public void limpia_tabla() {
        DefaultTableModel modelo = (DefaultTableModel) lo_pnl_grid_busq_ref_documento.TBL_referencia.getModel();
        ...
        public void keyPressed(KeyEvent ke) {
            if (ke.getKeyCode() == KeyEvent.VK_ENTER) {
                ...
                if (ke.getSource() == TXT_dato) {
                    if (lo_pnl_grid_busq_ref_documento.TBL_referencia.getRowCount() != 0) {
                        lo_pnl_grid_busq_ref_documento.TBL_referencia.requestFocus();
                        lo_pnl_grid_busq_ref_documento.TBL_referencia.changeSelection(0, 0, false, false);
                    } else {
                        TXT_dato.requestFocus();
                    }
                }
                if (ke.getSource() == lo_pnl_grid_busq_ref_documento.TBL_referencia) {
                    retorna();
                }
            }
    ...
    MouseListener MouseEvnt = new MouseListener() {
        ...
        public void mouseClicked(MouseEvent me) {
            if (me.getSource() == lo_pnl_grid_busq_ref_documento.TBL_referencia && me.getClickCount() == 2) {
                retorna();
            }
        }
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                dlg_busq_ref_documento dialog = new dlg_busq_ref_documento(new javax.swing.JFrame(), true);
