...
public class DAO_pedido {
    ...
    public boolean IST_pedido(BEAN_pedido OBJ_ped, JTable OBJ_pgp, double porcentaje_igv) throws SQLException {
        ...
        try {
            ...
            lq_stm.getConnection().rollback();
            if (e.getMessage().indexOf("pedido_" + gs_periodo + "_pkey") > 0) {
                if (go_fnc_mensaje.get_respuesta(0, "PEDIDO YA EXISTE ¿DESEA ACTUALIZAR Nro DOCUMENTO?") == 0) {
                    gs_parametros[3] = "DUPLICADO";
                }
            } else {
    ...
    public ResultSet SLT_grid_preliminar(String codigo_vendedor, String codigo_sector) {
        try {
            lq_stm = go_conexion_db.crearStatement();
            String SQL = "select * from slt_grid_preliminar('" + codigo_vendedor + "','" + codigo_sector + "','" + gs_periodo + "') "
                    + "as (codigo_operacion character(16),codigo_sucursal character(4),fecha_emision date,numero text,razon_social character varying(250),forma_pago character(2),nombre_vendedor character varying(250),descripcion character varying(250))";
            lq_rs = lq_stm.executeQuery(SQL);
            go_fnc_finaliza_conexion.finalizar(lq_stm, lq_stm.getConnection());
            if (lq_rs.next()) {
                return lq_rs;
            ...
            go_fnc_mensaje.GET_mensaje(2, ls_modulo, ls_capa, ls_clase, "SLT_grid_preliminar", e.getMessage());
        }
        return null;
    }
}
